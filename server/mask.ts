...
export function maskSecrets(text: string): string {
  ...
  // GitHub token families: personal, oauth, server, user, refresh
  masked = masked.replace(/gh[pousr]_[A-Za-z0-9]{36,}/g, '•••MASKED•••');
  ...
  masked = masked.replace(/AIza[0-9A-Za-z_\-]{35}/g, '•••MASKED•••');
  ...
  // Generic key/password/token parameter assignments with length >= 20
